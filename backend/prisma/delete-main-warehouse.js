...
async function main() {
  ...
  const classes = await prisma.academicClass.findMany({
    where: { branchId },
    include: {
      bookKits: { include: { items: { include: { subItems: true } } } },
    },
  })
  console.log(`Academic classes: ${classes.length}`)

  for (const cls of classes) {
    for (const bookKit of cls.bookKits ?? []) {
      for (const item of bookKit.items) {
        await prisma.bookKitSubItem.deleteMany({ where: { kitItemId: item.id } })
      }
      await prisma.bookKitItem.deleteMany({ where: { kitId: bookKit.id } })
      await prisma.bookKit.delete({ where: { id: bookKit.id } })
    }
