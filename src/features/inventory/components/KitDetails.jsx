...
function buildLinesFromKit(kit, branchId, branches = []) {
  ...
  return kit.items.map((item) => {
    ...
    return {
      ...
      price: Number(item.setPrice ?? item.price),
      branchStocks,
...
export default function KitDetails({ selectedClassId, selectedClassLabel, classData, branchId, branches = [], isSuperAdmin: isSuperAdminProp, onProductSaved }) {
  const academicKit = classData?.bookKit
  const notebookKit = classData?.notebookBookKit
  const kit = academicKit
  ...
  const badge = (() => {
    const a = kit?.badge ?? (hasSelectedClass ? 'Academic Kit' : '')
    if (notebookKit?.items?.length) return a ? `${a} + Notebooks` : 'Notebooks'
    return a
  })()
  const lastUpdatedTs = [kit?.lastUpdated, notebookKit?.lastUpdated].filter(Boolean).map((d) => new Date(d).getTime())
  const maxTs = lastUpdatedTs.length ? Math.max(...lastUpdatedTs) : null
  const lastUpdated = maxTs
    ? `Last updated: ${new Date(maxTs).toLocaleDateString('en-GB').replace(/\//g, '/')}`
    : ''

  const baseLines = useMemo(() => {
    const a = buildLinesFromKit(academicKit, branchId, branches)
    const n = buildLinesFromKit(notebookKit, branchId, branches)
    return [...a, ...n]
  }, [academicKit, notebookKit, branchId, branches])
  const lines = useMemo(
    () => baseLines.map((line) => ({
      ...line,
