...
const METHODS = [
  ...
  { id: 'ONLINE', label: 'Online', sub: 'Net banking / payment link', icon: 'language' },
  { id: 'CANARA_UPI', label: 'Canara Bank', sub: 'UPI to Canara account', icon: 'account_balance' },
  { id: 'UPI', label: 'UPI', sub: 'Direct UPI transfer', icon: 'qr_code_2' },
  { id: 'CARD', label: 'Card', sub: 'Credit or Debit card', icon: 'credit_card' },
]

export default function PaymentMethodGrid({ payment, onPaymentChange, amount, remarks, onRemarksChange }) {
  ...
  useEffect(() => {
    ...
    const next = METHODS.find((m) => m.id !== firstMethod)?.id
    if (!next || next === secondMethod) return
    onPaymentChange((prev) => ({ ...prev, secondMethod: next }))
  }, [enableSplit, firstMethod, secondMethod, onPaymentChange])
  ...
  return (
    <div>
      ...
      <p className="mb-4 text-sm text-on-surface-variant">
        Amount to collect: <span className="font-bold text-primary">₹{total.toFixed(2)}</span>
      </p>
      ...
      <div className="grid grid-cols-2 gap-3 md:grid-cols-3">
        {METHODS.map((m) => (
          ...
          >
            <input
              className="sr-only"
              name="admission-payment-method"
              type="radio"
              checked={firstMethod === m.id}
              onChange={() => {
                let nextSecond = secondMethod
                if (enableSplit && nextSecond === m.id) {
                  nextSecond = METHODS.find((x) => x.id !== m.id)?.id || ''
                }
                update({ firstMethod: m.id, secondMethod: nextSecond })
              }}
            />
            <div className="mb-3 flex items-start justify-between">
              <div className="rounded-xl bg-primary/10 p-2 text-primary">
                <span className="material-symbols-outlined text-lg" aria-hidden>{m.icon}</span>
              </div>
              <div
          ...
          <div className="relative ml-4 flex-shrink-0">
            <input
              ...
              onChange={(e) => update({
                ...
                firstAmount: e.target.checked ? payment.firstAmount : String(total),
              })}
              className="sr-only"
            />
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              <div>
                <label className="mb-1 block text-xs font-semibold text-on-surface-variant">
                  Amount via {methodById(firstMethod)?.label ?? 'First Method'}
                </label>
                <input
                  type="number"
                  min={0}
                  ...
                  onChange={(e) => {
                    ...
                    const clamped = Math.min(Math.max(raw, 0), total)
      ...
      <div className="mt-8">
        ...
        <textarea
          ...
          maxLength={200}
          rows={3}
          placeholder="e.g. Collected at front desk, fee waived partially…"
        ...
        />
        <p className="mt-1 text-right font-label text-[10px] text-on-surface-variant">{remarks.length}/200</p>
      </div>
    </div>
  )
}
