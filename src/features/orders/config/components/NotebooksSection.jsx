 ...
 * NotebooksSection
 *
 * Full Bundle — checkboxes to include/exclude each type at catalog default qty (no steppers).
 * Selected Sub-items — checkboxes plus steppers to change quantities; per-line ₹33 pricing.
 */
...
export default function NotebooksSection({
  ...
  const selectedBookCount = subItems.reduce((n, s) => n + (quantities[s.id] ?? s.quantity ?? 0), 0)
  ...
  const handleSectionToggle = (e) => {
    if (typeof onSectionEnabledChange === 'function') onSectionEnabledChange(e.target.checked)
  }

  const subtitle = !sectionEnabled
    ? 'Notebooks bundle is not included in this order. Tick the box to add and configure.'
    : bundleMode === 'full'
      ? 'Tick types to include. Quantities follow the class default — use Selected Sub-items to change counts.'
    ...
    <div className="rounded-3xl bg-surface-container-low p-4 md:p-8">
      <div className="mb-4 flex flex-col gap-4 md:mb-8 md:flex-row md:items-start md:justify-between">
        <div className="flex min-w-0 items-start gap-2 md:gap-3">
          <input
            className="item-checkbox mt-1 h-5 w-5 shrink-0 rounded border-outline-variant text-secondary focus:ring-2 focus:ring-secondary"
            type="checkbox"
            checked={sectionEnabled}
            onChange={handleSectionToggle}
            aria-label="Include notebooks bundle in this order"
          />
          <div className="rounded-xl bg-secondary/10 p-2">
            <span className="material-symbols-outlined text-secondary" aria-hidden>library_books</span>
          </div>
      ...
      {sectionEnabled && bundleMode === 'subitems' && (
      ...
      <div className="space-y-2">
        {subItems.map((sub) => {
          ...
          const isReduced = bundleMode === 'subitems' && enabled && currentQty < defaultQty
          const isIncreased = bundleMode === 'subitems' && enabled && currentQty > defaultQty
          ...
          return (
            ...
            >
              ...
              <div className="flex w-24 items-center justify-center">
                {bundleMode === 'full' ? (
                  <span
                    className={`text-sm font-bold tabular-nums ${enabled ? 'text-on-surface' : 'text-on-surface-variant'}`}
                    title={enabled ? 'Default quantity for this class' : 'Unchecked'}
                  >
                    {enabled ? currentQty : '—'}
                  </span>
                ) : (
        ...
        <div className="mt-4 flex items-center justify-between rounded-xl border border-secondary/20 bg-secondary/5 px-4 py-3">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs font-semibold text-secondary">
              {selectedBookCount} notebook{selectedBookCount === 1 ? '' : 's'} selected
            </span>
          </div>
          <div className="flex flex-col items-end">
