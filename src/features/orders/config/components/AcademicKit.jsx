...
function productPrice(item, selection) {
  ...
  if (isBundle) {
    if (selection.bundleMode === 'full') return Math.ceil(Number(item.setPrice ?? item.price ?? 0))
...
export default function AcademicKit({ kitItems, selections, onChange, loading }) {
    ...
    return (
      <div className="rounded-3xl bg-surface-container-low p-4 md:p-8">
        <div className="mb-4 flex items-center gap-3 md:mb-8">
          ...
          <div className="h-6 w-36 animate-pulse rounded-lg bg-surface-container" />
        </div>
    ...
    <div className="rounded-3xl bg-surface-container-low p-4 md:p-8">
      ...
      {kitItems.length === 0 ? (
        <div className="rounded-xl border border-outline-variant/20 bg-surface-container p-6 text-center">
          <span className="material-symbols-outlined mb-2 text-3xl text-on-surface-variant" aria-hidden>
            inventory_2
          </span>
          <p className="text-sm font-medium text-on-surface-variant">
            No academic items configured for this class.
          </p>
          <p className="mt-1 text-xs text-on-surface-variant/70">
            A Super Admin can configure the academic kit from the Stock Management screen.
          </p>
        </div>
      ) : (
        <div className="space-y-3">
                ...
                {selection.enabled && isBundle && selection.bundleMode === 'subitems' && subItems.length > 0 && (
                  <div className="mt-3 rounded-xl border border-outline-variant/20 bg-surface-container-low p-2.5">
                    <div className="mb-2 flex items-center justify-between rounded-lg bg-white px-3 py-1.5">
                      ...
                      <span className="text-xs font-bold text-primary">₹{Math.ceil(Number(item.setPrice ?? item.price ?? 0))}</span>
                    </div>
                      ...
                      {subItems.map((sub) => {
                        ...
                        return (
                          ...
                          >
                            ...
                            <span className="shrink-0 text-xs font-bold text-on-surface">₹{Math.ceil(Number(sub.price))}</span>
                          </label>
                        )
                      })}
                    </div>
