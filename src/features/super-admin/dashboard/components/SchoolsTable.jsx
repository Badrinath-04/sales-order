    ...
    <div className="overflow-hidden rounded-xl bg-surface-container-lowest">
      <div className="border-b border-outline-variant/10 px-8 py-6">
        ...
        <p className="mt-1 font-body text-sm text-on-surface-variant">
          Order volume and revenue snapshot across the network.
        </p>
      </div>
          ...
          <tbody className="divide-y divide-surface-container-highest">
            {schools.map((row) => (
              <tr key={row.id} className="bg-surface-container-lowest font-body text-sm">
                ...
                <td className="px-6 py-4 text-on-surface">{row.students.toLocaleString('en-US')}</td>
                <td className="px-6 py-4 font-medium text-on-surface">{formatCurrency(row.revenue)}</td>
